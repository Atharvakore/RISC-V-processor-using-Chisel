...
class RV32I (
  ...
  switch(control_unit.io_ctrl.reg_write_sel) {
    ...
    is(REG_WRITE_SEL.MEM_OUT_SIGN_EXTENDED) {
          // Sign extend the data accordingly(if read half word or read a byte) and then store in register
          when ( io.instr(14, 12) === "b000".U ) {
          // 8 bits wide data
            io_reg.reg_write_data := Cat(Fill(24, io_data.data_rdata(7)), io_data.data_rdata(7,0))
        } 
        when ( io.instr(14, 12) === "b001".U ){
          // 16 bits wide data
            io_reg.reg_write_data := Cat(Fill(16, io_data.data_rdata(15)), io_data.data_rdata(15,0))
        }
        .otherwise {io_reg.reg_write_data := io_data.data_rdata}
    }
